...
### Pending ratification — corpus reconciliation ([`DEC-0018`](decisions/DEC-0018-20260730-duplicate-decision-number-reconciliation.md), [`DEC-0019`](decisions/DEC-0019-20260730-counter-tooling-branch-and-worktree-blindness.md))
...
- **The claimed-id set spans refs; the corpus boundary stops at nested checkouts** ([`DEC-0019`](decisions/DEC-0019-20260730-counter-tooling-branch-and-worktree-blindness.md), clauses DEC-0019_1 and DEC-0019_2) — the two tools enforcing counter discipline both mistake one working tree for the corpus, failing in opposite directions. `claim_id.py` **under-reports**: `scan_real_max()` is an `os.walk()` that cannot see a branch, and `.counters/dec.txt` forks with the branch, so both inputs go stale together — which is how 0014 was issued thirteen days after it was spent. `validators.py`'s counter-atomicity check **over-reports**: it descends into `.claude/worktrees/`, finding every record twice. Claims and validation walks now read git refs, with an announced filesystem fallback where git is unavailable; the nested-checkout boundary moves to module level so every walk shares it.
