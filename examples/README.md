# examples/ — Reference implementations + adopter onboarding

Concrete, copy-pasteable starting points for adopters.

## Planned contents

| Document | Purpose |
|---|---|
| `reference-implementations.md` | Catalog of known SliceOps runtime implementations (vendor runtimes + adapters), their Capa B compliance level, and selection guidance. Implementations are architectural peers — none is "the official" runtime. |
| `onboarding-day-1.md` | Simplified onramp ("Day 1 SliceOps") — the minimum to start honoring the principles without reading the full spec. |
| `adopter-customization-guide.md` | What adopters may customize (rename folders, extend the entity catalog, add vertical packs) while staying SliceOps-compliant, and what they may not (remove canonical entities, conflict canonical semantics). |

## Status

Scaffolding in progress. Content pending the Capa B.1 drafting batch.

## Note

Examples must be vendor-neutral and platform-agnostic (P8). Tutorial content that assumes a specific platform's features does not belong here.
