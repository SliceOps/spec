...
> Formerly the "RFC process". The term **"RFC" is retired** from the SliceOps vocabulary (v1.2.0 naming homologation): a proposal IS a **pending DecisionRecord** (`DEC-P-`, `status: pending`) — the lifecycle lives in the prefix, not in a folder or a separate artifact type. See `../spec/v1.2.0/naming.md`.
