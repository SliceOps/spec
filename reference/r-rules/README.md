# R-rules — Capa B.1 (v1.0)

The **R-rules system**: enforced merge gates in CI that validate methodology metadata. SliceOps IP (Capa B.1); documentation under CC BY 4.0, code templates under MIT (final terms pending IP/Legal — see `../../governance/IPR_POLICY.md`).

Each R-rule is a **hard gate**: a PR violating any rule does not merge. R1–R14 are the **canonical starter pack** — vendor-neutral, stack-agnostic, adopter-instantiable. Adopters add R15+ for their own stack/domain (Capa C.2). The starter rules trace to canonical principles; an R-rule with no principle backing is a retirement candidate.
...
3. Add adopter-specific R15+ rules (Capa C.2) for stack patterns.
