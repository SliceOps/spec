...
4. **Canonical ID patterns**: `sliceId` uses `BL-NN[.SEC-NN].SL-NNN[a-z]`; decision refs use `DEC-YYYY-MM-DD-slug` (lifecycle prefixes `DEC-P-`/`DEC-D-` included; legacy `DR-` read-tolerated for archives) or counter-based ids (`DEC-NNN`, `INS-NNN`).
