...
status: approved
kind: constitutive          # proposed by this very DEC (DEC-0008.3) — dogfooding
created: 2026-07-12
updated: 2026-07-12
...
`_index.md` (DEC-0010) — split from this record by the clause rule of DEC-0008.9. These are breaking changes to the framework contract, so this ships as
...
### DEC-0008.1 — The catalog is presented on the cognition cycle
...
### DEC-0008.2 — Three entities renamed to plain words (catalog stays 13)
...
| 05 | **Goal** | `GOAL-` | A measurable objective with a stated `horizon` (now / quarter / year / multi-year) and a `measure`. REQUIRES `decided-by:` — the decision that created it (DEC-0008.4). Vision is a multi-year Goal with a narrative body (DEC-0008.7.1). | Aim | `proposed → active → achieved / abandoned` (abandonment requires rationale) |
...
| 09 | **Priority** *(was ActivePriority)* | `PRI-` | A ranked commitment of focus: what is being worked now/next and in which order. REQUIRES `serves-goal:` and an integer `rank` unique within `(owner, horizon)` (DEC-0008.4) — buckets do not order; ranks do. The name carries no state: `status:` does. | Focus | `open → in-progress → blocked / resolved` |
...
| — | *Slice coordinate* | `SLC[n]SEC[n]BL[n]` | Not an entity: the **work coordinate** (slice within section within block) used in branches, commits, trackers and `originating_slice:` (DEC-0008.6). | Act — the address of action | not applicable |

### DEC-0008.3 — DecisionRecord gains a kind axis; the edges carry the truth
...
### DEC-0008.4 — The pyramid becomes mandatory (goal provenance and priority ordering)
...
### DEC-0008.5 — One universal ID grammar for all entity artifacts
...
### DEC-0008.6 — The slice coordinate: `SLC`, letters as separators
...
- `SLC` leads — the slice is the framework's atomic unit and its namesake. Minimum
  widths: `SLC` 4 digits, `SEC`/`BL` 2 digits; all unbounded per DEC-0008.5.1.
...
### DEC-0008.7 — Three conventions (no new entities)
...
2. `naming.md`, glossary, entity catalog and templates rewritten from the DEC-0008.2.1 table
...
The canonical catalog table is DEC-0008.2.1 (single source — naming.md and the entity catalog
...
template heading (DEC-0008.7.4).

### DEC-0008.9 — Clause identifiers and the independence test

Resolutions inside a DecisionRecord are **clauses**, cited as `DEC-NNNN.n` (this record's
are DEC-0008.1 … DEC-0008.9) — never as bare "Decision n", which collides with the
record numbering itself. Clause identifiers are citable from any document and are valid
targets of supersession edges: a future record may declare `supersedes: [DEC-0008.3]`
without touching the sibling clauses.

**The independence test** — what is a clause and what is its own record:

- **Clause** (`DEC-NNNN.n`): shares the deliberation context and alternatives-set of its
...
- **E — Add Vision/Plan/Product entities**: rejected — Vision maps to multi-year Goals
  (DEC-0008.7.1); the absence of Plan is a philosophical position to defend, not a gap (principle P5 — Stage as DAG-Derived View:
...
- Principle P5 (Stage as DAG-Derived View) — reaffirmed by alternative E and DEC-0008.7: plans are derived, never decreed.
