...
Governed by [`DEC-0008`](../../decisions/DEC-0008-20260712-cognitive-cycle-and-universal-id-scheme.md) (the cognition cycle and the universal identifier scheme), with [`DEC-0009`](../../decisions/DEC-0009-20260712-handoffs-as-a-contextpack-kind.md) (handoffs), [`DEC-0010`](../../decisions/DEC-0010-20260712-corpus-index-as-reserved-name-infrastructure.md) (the corpus index), [`DEC-0011`](../../decisions/DEC-0011-20260713-canonical-corpus-container-and-layout.md) (the canonical container and layout, §7), [`DEC-0012`](../../decisions/DEC-0012-20260713-catalog-amendments-mental-model-and-policy.md) (MentalModel + Policy catalog amendments), [`DEC-0013`](../../decisions/DEC-0013-20260713-clause-identifier-separator-underscore.md) (clause-identifier separator, §3) and [`DEC-0014`](../../decisions/DEC-0014-20260715-slc-coordinate-subslice-and-alpha-sections.md) (slice-coordinate sub-slice suffix + alphabetic section codes, §5). The complete catalog table with definitions is clause DEC-0008_2_1 as amended by DEC-0012 — the entity catalog (`../../reference/entity-catalog/`) is rewritten from it.
...
| — | *slice coordinate* (not an entity) | `SLC[n][a-z]SEC[n\|A-Z]BL[n]` (§5) | `BL-XX.SEC-XX.SL-XXX[a-z]` (dotted form) |
...
Work coordinates (clause DEC-0008_6, extended by clause DEC-0014_1/DEC-0014_2) replace the dotted Slice ID. **Letters are the separators** — no dots (fragile in git references and `.md` filenames), no inner hyphens (the hyphen is the grammar's field separator):

```
SLC0012SEC03BL02        full coordinate (slice ∈ section ∈ block)
SLC0034                 simple form — the suffix, SEC and BL are optional
...
Grammar: `SLC\d{4,}[a-z]?(SEC(\d{2,}|[A-Z]{2,}))?(BL\d{2,})?`.

- **Sub-slice suffix** — one *optional lowercase letter* right after the slice number (`SLC0010b`) marks a slice that split off an existing one mid-implementation without renumbering its neighbours. Exactly one letter (`SLC0010ab` is invalid); lowercase only (`SLC0010B` is invalid). Restores the `[a-z]` tail the legacy dotted form carried (clause DEC-0014_1; precedent: the evidence `sliceId` pattern in [`../../reference/evidence/evidence-v1.md`](../../reference/evidence/evidence-v1.md)).
- **Section codes: numeric or alphabetic, pure** — a `SEC` code is *all digits* (`SEC03`) **or** *all uppercase letters* (`SECAPI`), never mixed (`SECA1` is invalid). An alphabetic code **must not contain the substring `BL`** — that is the block qualifier's opener and the one source of parse ambiguity, so a code like `SECTABLE` is rejected at write time (clause DEC-0014_2). The block qualifier `BL` stays **numeric** (`BL\d{2,}`).

Minimum widths: `SLC` 4 digits, `SEC`/`BL` 2 (an alphabetic `SEC` code is ≥2 letters) — all unbounded per §2. In frontmatter: `originating_slice: SLC0012SEC03BL02`. In branches, commits and pull-request titles: the bare coordinate. As a materialized file it joins the universal grammar: `SLC0012SEC03BL02-20260712-slug.md`. Slice coordinates inside **merged git history are immutable** (same standing as archives) — alias maps cover them; trackers, ledgers, frontmatter and living documents are rewritten. Retirement of the dotted recognizer (clause DEC-0008_6) binds a corpus only once it adopts a spec version ≥2.0.1; corpora pinned to 2.0.0 in `sliceops.json` migrate when they raise the pin (clause DEC-0014_3).

The *why* of the sub-slice — that it is emergent granularity discovered in execution (the plan commits to structure; execution discovers the leaves), and that its **rate** across a corpus is a health signal for planning altitude — is a principle, not a naming rule: see [`principles.md`](principles.md) P4 (atomicity under emergence) and P5 (the plan is a hypothesis), and clause DEC-0014_4. Reserve the suffix for *emergent* work bound to a parent; plan foreseeable batches as numbered siblings. This section stays mechanical — the concept is not copied here (Context Discipline, P12).
...
| `BL-XX.SEC-XX.SL-XXX[a-z]` | `SLC…[a-z]SEC…BL…` | slice coordinate (§5); the sub-slice suffix and alphabetic section codes are preserved across the migration (DEC-0014) |
