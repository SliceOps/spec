...
### Block Retrospective
- **Definition**: Mandatory ritual at Block close. Includes velocity recalibration, a Cross-DEC Consistency Check section, and InsightRecord harvesting. Materialized as an **OutcomeRecord with `kind: retrospective`** (`OUTC-` — the former standalone `BR-` prefix is retired).
- **Origin**: P5, P8
...
- **Definition**: **Renamed to MentalModel** in v2.0.0 (clause DEC-0008_2 as amended by clause DEC-0012_1) — see *MentalModel*. The `CF-` prefix is retired.
...
- **Origin**: Catalog split; P2/P1; naming homologation; kind axis and clause identifiers (`DEC-0008_3`/`DEC-0008_9`: `kind: constitutive | strategic | tactical` with goal edges; clauses cited `DEC-NNNN_n`)
...
- **Definition**: P6. Each slice produces evidence in 4 categories (functional, quality, decision, provenance) plus security (P7). Hard gates enforce; slices without complete evidence don't merge. Canonical record format: evidence.v1 (`reference/evidence/`).
- **Origin**: P6
...
- **Definition**: **Renamed to MentalModel** by clause DEC-0012_1 — see *MentalModel*. "Frame" was the interim v2.0.0 design name (clause DEC-0008_2, between CognitiveFramework and the final name); the `FRAME-` prefix is retired alongside `CF-`.
...
### InsightRecord
- **Definition**: Cognitive entity. An empirical observation captured from a slice. Append-only. Raw material for LearningPatterns. Naming: `INS-<id>-slug.md` (canonical prefix `INS-`; id scheme per store, see `naming.md` §2).
- **Origin**: Catalog split; P8
- **Aliases prohibited**: `IN-`, `IR-` (retired prefixes)
- **Cross-references**: P8, LearningPattern, Recursive Learning, Naming (canonical prefixes)
...
- **Definition**: **Renamed to Conclusion** in v2.0.0 (clause DEC-0008_2) — see *Conclusion*. The `LP-` prefix is retired.
- **Origin**: Naming homologation (v2.0.0)
...
### Policy
- **Definition**: Cognitive entity 14 (clause DEC-0012_2). An operating rule with scope and enforcement — how an environment, an agent, a corpus or a session MUST operate; verifiable, blockable. One record per policy (`POL-NNNN-YYYYMMDD-slug.md`) with `scope: environment | agent | corpus | session` (vendors MAY extend the values, Layer C), `enforced-by:` and `severity: block | warn`. Transversal: policies are what the audit plane's machine gates enforce. The corpus `_policies.md` is the DERIVED summary of the active records (clause DEC-0012_3) — never hand-edited. Boundary: a Value is a terminal criterion, a Preference is a taste, a **Policy is enforceable**.
- **Origin**: DEC-0012_2
- **Cross-references**: Value, Preference, Audit plane

### Priority
- **Definition**: Cognitive entity 09. A **ranked commitment of focus toward a goal**: `serves-goal:` and an integer `rank` (unique within owner and horizon) are mandatory — "priority over what" is only answerable as an ordering of focus toward Goals. Naming: `PRI-<id>-slug.md`. Renamed from "ActivePriority" (clause DEC-0008_2): an entity name must never contain a state. Briefs, handoffs, checklists and drafts are NOT priorities (they are ContextPacks, Capability components, or knowledge-layer content).
- **Origin**: Naming homologation (v2.0.0); pyramid edges (clause DEC-0008_4)
...
### Slice coordinate (SLC)
- **Definition**: The canonical work coordinate `SLC…[a-z]SEC…BL…` (slice ∈ section ∈ block) — grammar `SLC\d{4,}[a-z]?(SEC(\d{2,}|[A-Z]{2,}))?(BL\d{2,})?`. `SLC` leads (minimum 4 digits); an **optional one-letter lowercase sub-slice suffix** may follow the slice number (`SLC0010b`, clause DEC-0014_1); the **section code is numeric or alphabetic, pure** — all digits or all uppercase letters, never mixed, and an alphabetic code may not contain the substring `BL` (clause DEC-0014_2); the **block qualifier `BL` is numeric**. `SEC` and `BL` are optional qualifiers. A provenance carrier in branch names, commit messages, PR titles, and DEC frontmatter (`originating_slice:`); as a materialized file it joins the universal grammar (`SLC0012SEC03BL02-20260712-slug.md`). Letters are the separators — no dots, no inner hyphens. Normative source: [`naming.md`](naming.md) §5.
- **Origin**: P1; clause DEC-0008_6, extended by clause DEC-0014_1/DEC-0014_2
