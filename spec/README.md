# spec/ — Versioned canonical framework spec

The authoritative SliceOps™ framework specification, versioned with SemVer.

- `v1.0.0/` — first published spec version (content drafting in progress)
- `latest` → symlink to current GA version (added at first release)

## Status

Scaffolding in progress. Canonical content (principles, glossary, topics, IP boundary) is being drafted and will be ported here from canonical sources.

## What lives here

| Document | Purpose |
|---|---|
| `principles.md` | The 12 canonical principles (Layer A) — non-negotiable framework core |
| `glossary.md` | Canonical vocabulary (P12 substrate) |
| `topics.md` | Canonical topic taxonomy |
| `ip-boundary.md` | Layer A/B/C three-layer IP boundary and sub-numbering taxonomy |

## What does NOT live here

Working drafts (those live in `decisions/rfcs/`), adopter-specific customizations (those live in the adopter's own brain), vendor runtime implementation details.
