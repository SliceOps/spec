...
SemVer. Breaking changes to the framework contract lead to a new major. Backward-compatible additions lead to a minor. Clarifications/typos lead to a patch. A new `vX.Y.Z/` directory is created for any major/minor; prior versions (`../v2.0.1/`, `../v2.0.0/`, `../v1.1.0/`, `../v1.0.0/`) are retained frozen for audit; v1.2.0 was a working cut that never published (re-issued as v2.0.0). This v2.1.0 is a backwards-compatible minor over v2.0.1 (the slice-coordinate grammar extension of DEC-0014). `latest →` symlink points at the current version.
