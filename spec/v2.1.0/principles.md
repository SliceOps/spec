...
- The DEC schema records the ratifying human in `approver` (recommended on `status: ratified`). In projects with more than one maintainer, `approver` MUST differ from `owner`. Single-maintainer projects MUST record self-ratification explicitly (`approver` == `owner`) — a disclosed limitation, consistent with DISCLOSURE.md, not a silent one.
...
**Clarification (atomicity under emergence — the sub-slice)**: A slice's coordinate is claimed when the slice is cut, but work is discovered by doing it. When execution surfaces a unit that genuinely belongs to the slice — a necessary adjacent fix, an enumerable leaf of a batch — that unit earns its own atomic identity (its own evidence, merge, and DAG node) while staying bound to its origin, expressed as a **sub-slice**: the one-letter suffix on the coordinate (`SLC0010b`; naming §5, clause DEC-0014_1). This does not violate P4 — it is P4 *applied under emergence*. The two alternatives are both worse: burying the discovered work inside the parent destroys the parent's atomicity and smears its evidence; minting an unrelated top-level coordinate severs the provenance the audit plane (P2) exists to preserve. The suffix — never a renumber — is what expresses it, because coordinates are claimed monotonically and merged history is immutable (naming §5). Reserve the suffix for **emergent** work bound to its parent; foreseeable fan-out (a batch whose leaves you can enumerate before starting) is planned as numbered siblings, not sub-sliced after the fact.
...
**The plan is a hypothesis; execution discovers the leaves.** A plan is stated at one altitude and is *deliberately* coarser than reality — detail that changes no decision is waste. The true, final granularity of the work is knowable only by doing it: the fix that exists only because a tool broke on a diff not yet generated cannot be enumerated in advance. So the plan commits to *structure* and execution discovers the *leaves*; a **sub-slice** (P4) is the notation for a leaf that emerged after its parent's coordinate was already spent. This is not a planning failure — it is the correct division of labor between planning and execution, and it is why plans are derived and cheap rather than exhaustive.

**Sub-slice rate is a health signal.** Because a sub-slice is the plan correcting itself against reality, the *rate* at which a corpus produces them measures its planning altitude. A low rate, concentrated in inherently-emergent work (tooling, cleanup, meta-work), is healthy: slices were cut at the right granularity and the suffix absorbs only the irreducible tail. A rising rate — or sub-slices spreading into the plannable core of the build — means slices are being cut too coarse and the suffix is compensating: a prompt to plan finer, never a mechanism to lean on. The sub-slice is therefore an *instrument*, not just a coordinate; the framework asks each corpus to keep its rate observable (a sweeper metric, not a hard gate — degradation announced, P9).
