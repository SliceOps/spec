...
Core spec documents present. The 13-entity catalog, the knowledge-category taxonomy, the R-rule starter templates, frontmatter schemas, file templates, agent-skill concept, B.2 patterns, evidence.v1, measurement, and Layer 3 validator specs are materialized in `../../reference/`. Sizing (token-band plus context-band), Model Triage, Context Router, and the development-model characterization are documented alongside.
