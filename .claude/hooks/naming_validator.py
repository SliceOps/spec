...
# Slice coordinate (DEC-0008.6, extended by DEC-0014): filename and bare
# frontmatter forms. DEC-0014.1 adds an OPTIONAL one-letter lowercase sub-slice
# suffix ([a-z]) after the slice number; DEC-0014.2 admits ALPHABETIC section
...
# anti-BL constraint (DEC-0014.2 — an alphabetic code may not contain "BL") can
...
# Legacy dotted Slice ID (BL-XX.SEC-XX.SL-XXX[a-z]), retired by DEC-0008.6.
...
def _alpha_sec_has_bl(m):
    """DEC-0014.2 anti-BL constraint: True when the SLC coordinate's PARSED
...
def validate_file(path, text, tolerate_legacy=False, transition=False):
    ...
    # 11) Slice coordinate — frontmatter form (DEC-0008.6, extended by DEC-0014)
        ...
        if SLC_LEGACY_DOTTED.match(slice_val):
            ...
            if not m:
                errs.append(f"{p}: originating_slice '{slice_val}' does not match the SLC coordinate "
                            f"grammar SLC\\d{{4,}}[a-z]?(SEC(\\d{{2,}}|[A-Z]{{2,}}))?(BL\\d{{2,}})? ({DEC8}.6/{DEC14}.1/.2)")
                ...
                errs.append(f"{p}: originating_slice '{slice_val}' has an alphabetic section code "
                            ...
                            f"ambiguity source). Pick section letters that do not spell 'BL' ({DEC14}.2)")

    # 12) Slice coordinate — filename form (DEC-0008.6, extended by DEC-0014)
    if SLC_LEAD.match(base):
        ...
        if not m:
            ...
            errs.append(f"{p}: slice-coordinate filename has an alphabetic section code containing "
                        ...
                        f"Pick section letters that do not spell 'BL' ({DEC14}.2)")
